...
function init() {
    ...
    xmlhttp.open("GET", "/GENERATED/LOGIN/" + username + "/" + pass, false);
    xmlhttp.send();
...
function sendReport() {
    if (criteria[0] && criteria[1] && criteria[2] && criteria[4] && criteria[5]) {
        ...
        xmlhttp.open("GET", "/GENERATED/REPORT/" + session + "/" + flags + "/" + reported + "/" + reason + "/" + previousRank.options[previousRank.selectedIndex].value + "/" + newRank.options[newRank.selectedIndex].value, false);
        xmlhttp.send();
